<x-voter-layout>
    <div class="max-w-4xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Active Elections</h1>
        ...
        @if($elections->isEmpty())
             <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-8 text-center border border-gray-100 dark:border-slate-700">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">No active elections</h3>
                <p class="mt-2 text-gray-500 dark:text-gray-400">There are currently no elections open for voting. Please check back later.</p>
             </div>
        @else
                        ...
                        <div class="p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 rounded-full text-xs font-semibold tracking-wider uppercase">
                                        Voting Open
                                    </span>
                                ...
                                @if($hasVoted)
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if($election->results_published)
                                            <a href="{{ route('voter.results', $election) }}" class="mt-2 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 text-sm font-medium">
                                                View Results &rarr;
                                            </a>
                                        @endif
                                    </div>
